...
4. **阻塞（Blocked）**：线程等待获取锁或其他资源，无法继续执行。
5. **等待（Waiting）**：线程等待另一个线程的通知，可以通过调用wait()、join()等方法进入等待状态。
6. **超时等待（Timed Waiting）**：线程等待一段时间后自动返回，可以通过调用sleep()、wait(long timeout)等方法进入超时等待状态。
7. **终止（Terminated）**：线程执行完毕或被强制终止，进入终止状态。

## 线程安全问题
线程安全问题主要包括以下几种：
1. **竞态条件（Race Condition）**：多个线程同时访问共享资源，导致数据不一致。
2. **死锁（Deadlock）**：多个线程相互等待对方释放资源，导致程序无法继续执行。
3. **活锁（Livelock）**：多个线程不断地改变状态以响应其他线程的变化，但始终无法取得进展。
