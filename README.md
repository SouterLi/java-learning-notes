# java-learning-notes
大家好，这里是南泽。   
我是一名普通的java程序员，这个是我的Java学习笔记，如果你正在备战面试或者学习 java，相信它会对你有帮助。
如果你也有好的学习笔记，欢迎提交 PR，大家一起进步。

## 数据库
### MySQL
- [MySQL基础知识总结](database/mysql/mysql.md)
- [MySQL常见面试题](database/mysql/mysql-questions.md)
- [数据库查询性能优化 - 未完成](database/mysql/mysql-performance.md)
- [数据库事务](database/mysql/mysql-transaction.md)
### Redis
- [Redis 基础知识总结](database/redis/redis-basic.md)
- [Redis 实现分布式锁](database/redis/redis-distributed-lock.md)
- [Redis 高可用方案](database/redis/redis-high-availability.md)
### MyBatis
- [MyBatis 知识总结](database/mybatis.md)
### Elasticsearch
- [Elasticsearch知识总结](database/elasticsearch.md)

## 并发编程
- [Java常见的锁](concurrency/lock.md)
- [Java多线程基础知识总结](concurrency/thread.md)
- [Java 原子类](concurrency/automic.md)

## 分布式系统
- [分布式系统知识总结](distributed-system/distributed-system.md)
- [分布式锁](distributed-system/distributed-lock.md)
- [分布式事务](distributed-system/distributed-transaction.md)
- [分布式 ID](distributed-system/distributed-id.md)
- [高可用系统设计](distributed-system/high-availability.md)
- [Kafka 知识总结](distributed-system/kafka.md)

## Java基础
- [Java基础知识总结](java/java-basic.md)
- [Collection 知识总结](java/collection.md)
- [Map 知识总结](java/map.md)
- [Java设计模式](java/design-model.md)

## JVM
- [JVM内存模型](jvm/jvm-memory-model.md)
- [JVM垃圾回收](jvm/gc.md)
- [JVM类加载机制](jvm/classLoading.md)

## Spring
- [Spring基础知识总结](spring/spring.md)
- [Spring boot 知识总结](spring/springboot.md)

## 系统设计
- [如何设计一个高并发写入数据的系统](system-design/high-concurrency-write.md)
- [如何设计一个红包系统](system-design/red-packet-system.md)

## 算法与数据结构
- [算法与数据结构知识总结](algorithm.md)
